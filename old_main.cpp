...
void ModSwitchData( u8 ModField, u8 RmField, u8 Sbit, u8 Wbit, u32 *inst_i, memory_t ReadIn )
{
  const char *RmStr;
  switch( ModField )
  {
    ...
    case(1):{
      ...
      if(Sbit)
      {
        s16 Data = Wbit ? *(s16 *)&ReadIn.Memory[(*inst_i)+3] : (s16)(*(s8 *)&ReadIn.Memory[(*inst_i)+3]);
        RmStr = RmLut[RmField];
        ...
        u16 Data = Wbit ? *(u16 *)&ReadIn.Memory[(*inst_i)+3] : (u16)(*(u8 *)&ReadIn.Memory[(*inst_i)+3]);
        RmStr = RmLut[RmField];
        RMPrintDisp( RmStr, DispValue);
        printf(", ");
        printf("%d", Data);
    ...
    case(3):{
      if( Wbit )
      {
        ...
        {
          u16 Data = *((u16 *)&ReadIn.Memory[(*inst_i)+2]);
          printf( "%s, %d\n", RmStr, Data);
          (*inst_i)+=2;
        }
      }
      else
      {
        ...
        {
          u8 Data = *((u8 *)&ReadIn.Memory[(*inst_i)+2]);
          printf( "%s, %d\n", RmStr, Data);
        }
        (*inst_i)+=1;
      }
...
void ModSwitchWithReg( u8 ModField, u8 RegField, u8 RmField, u8 Wbit, u8 Dbit, u32 *inst_i, memory_t ReadIn)
{
  const char *RmStr;
  const char *RegStr;
  ...
  switch( ModField )
  {
    case(0):
    {
      ...
      {
        s16 DispValue = *((s16 *)&ReadIn.Memory[(*inst_i)+2]);
        (*inst_i)+=2;
        RegStr = Wbit ? RegistersWide[RegField] : RegistersSingle[RegField];
        if( Dbit )
        {
    ...
    case(1):
    {
      s8 DispValue = *((s8 *)&ReadIn.Memory[(*inst_i)+2]);
      (*inst_i)+=1;
      RmStr =  RmLut[RmField];
      RegStr = Wbit ? RegistersWide[RegField] : RegistersSingle[RegField];
      if( Dbit )
...
int main( s32 ArgCount, char **Args )
{
  ...
  if( ArgCount > 0 )
  {
    ...
    for(u32 inst_i = 0; inst_i < ReadIn.Size; inst_i+=2)
    {
      ...
      PrintBinaryU8(FirstEight);
      PrintBinaryU8(SecondEight);
      switch( FirstEight & 0b11110000 )
      {
        ...
        case( 0b10000000 ):
        {
          ...
          if( (FirstEight & 0b01111100) == 0)
          {
            ...
            u8 Sbit = (FirstEight & 0b00000010) >> 1;
            u8 Wbit = (FirstEight & W_MASK);
            u8 ModField = (SecondEight & MOD_MASK) >> 6;
            u8 RmField = (SecondEight & RM_MASK);
            /*
            ...
            PrintBinaryU8(FirstEight);
            PrintBinaryU8(SecondEight);
            PrintBinaryU8(ReadIn.Memory[inst_i+2]);
            PrintBinaryU8(ReadIn.Memory[inst_i+3]);
            PrintBinaryU8(ReadIn.Memory[inst_i+4]);
            */
            switch( OPCode )
            {
              case(0):{
                printf("add ");
                ModSwitchData( ModField, RmField, Sbit, Wbit, &inst_i, ReadIn );
              }break;
          ...
          switch(ModField)
          {
            ...
            case(0):{
              if( RmValue != 6 )
              {
                if( Wbit )
                {
                  s16 DataValue = *((s16 *)&ReadIn.Memory[inst_i+2]);
                ...
                {
                  s8 DataValue = *((s8 *)&ReadIn.Memory[inst_i+2]);
                  RMPrintDisp(RmLut[RmValue],0);
                  printf(", %s %d",ByteOrWord, DataValue);
                  inst_i+=1;
                }
            ...
            case(1):{
              ...
              if( Wbit )
              {
                s16 DataValue = *((s16 *)&ReadIn.Memory[inst_i+3]);
              ...
              {
                s8 DataValue = *((s8 *)&ReadIn.Memory[inst_i+3]);
                RMPrintDisp(RmLut[RmValue], DispValue);
                printf(", %s %d",ByteOrWord, DataValue);
                inst_i+=1;
              }
            ...
            case(2):{
              ...
              if( Wbit )
              {
                s16 DataValue = *((s16 *)&ReadIn.Memory[inst_i+4]);
                RMPrintDisp(RmLut[RmValue], DispValue);
                printf(", %s %d",ByteOrWord, DataValue);
                inst_i+=2;
              }
              ...
              {
                s8 DataValue = *((s8 *)&ReadIn.Memory[inst_i+4]);
                RMPrintDisp(RmLut[RmValue],DispValue);
                printf(", %s %d",ByteOrWord, DataValue);
                inst_i+=1;
              }
        ...
        case( 0b10110000 ):
        {
          ...
          {
            s16 Data = ReadIn.Memory[inst_i+1];
